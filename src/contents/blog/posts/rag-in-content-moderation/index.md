...
description: |-
  Retrieval-augmented generation (RAG) has become a common architecture for AI systems that need to ground their outputs in an existing body of knowledge. Rather than relying solely on a model's parametric memory, RAG retrieves relevant documents from a database at inference time and injects them into the model's context. The model reasons from those retrieved examples rather than from rules encoded during training.
